...
def build_prompt(name, cuisine, city, address, contacts, locale, with_ordering):
    ...
    if with_ordering:
        menu = ('Блок "Меню" (id="menu", Grid на 6 карточек): Названия блюд, описание, цены. '
                'Корневой элемент КАЖДОЙ карточки блюда обязан иметь атрибуты '
                f'data-dish="<название блюда на языке {lang}>" data-price="<цена числом, без валюты>" '
                f'data-currency="{locale.currency_symbol}". '
                'Кнопок заказа в карточки не добавляй — их добавит отдельный скрипт.')
    else:
        menu = 'Блок "Меню" (id="menu", Grid на 3-4 карточки): Названия блюд, описание, цены.'
    ...
    if locale.bilingual:
        ...
        logic = (f'Внутри <script> напиши логику переключателя языков: все тексты меняются между {lang} '
                 'и английским. Тексты придумай сам.')
    else:
